...
class yaml_config:
    ...
    def dumpToYaml(self,fname):
        with open(fname,'w') as fout:
            yaml.dump(self.default_options,fout)
            yaml.dump(self.sca_variables,fout)
            yaml.dump(self.toa_tot_variables,fout)
...
filename='config_full.yaml' #changeable by parser (later)
...
#Changes according to parser HERE (don't forget to load sca and tot toa variables, in case)
...
def run_test():
    ...
    if testswitches['IV_curve']==True:
        print('HERE!')
        (voltArray,currArray)=IV_curve.IVcurve(5, 20, 0.001,3, 0.5) #gpib_adr, vMax, complianceCurrent, stepSize, delay_between_steps
    ...
    if options.dataNotSaved==False:
        while True:
            ...
            rawFileName=glb_options['outputRawDataPath']+"/Module"+glb_options['moduleNumber']+"_"
            ...
            if False: #os.path.exists(rawFileName): #to avoid overwriting
                ...
                if glb_options['storeYamlFile']==True:
                    yamlFileName=glb_options['outputYamlPath']+"/Module"+glb_options['moduleNumber']+"_"
